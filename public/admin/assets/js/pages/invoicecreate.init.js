...
document.querySelector("#profile-img-file-input").addEventListener("change", function () {
  ...
  reader.addEventListener("load", function () {
    preview.src = reader.result; //localStorage.setItem("invoiceLogo", reader.result);
  }, false);
...
if (localStorage.getItem("invoice_no") === null && localStorage.getItem("option") === null) {
  ...
  viewobj = invoices.find(function (o) {
    return o.invoice_no === invoice_no;
  });
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  formEvent.addEventListener("submit", function (event) {
    event.preventDefault(); // get fields value
    ...
    var notes = document.getElementById("exampleFormControlTextarea1").value; // get product value and make array
    ...
    if (formEvent.checkValidity() === false) {
      ...
      if (options == "edit-invoice" && invoice_no == i_no) {
        objIndex = invoices.findIndex(function (obj) {
          return obj.invoice_no == i_no;
        });
